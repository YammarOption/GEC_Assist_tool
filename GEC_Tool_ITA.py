...
skiptrainer=["Rivale Opzionale".lower(),"Rivale".lower()]
...
def create_route_window(route,itemsinroute,trainerinroute,eventsinroute):
    ...
    for floor in floors:
        ...
        if floor in items and len(items[floor])>0:
            layout.append([sg.Text("Oggetti",font=("Arial",20,"bold"),expand_x=True,expand_y=True,justification='left')])
        ...
        if floor in trainers and len(trainers[floor])>0:
            layout.append([sg.Text("Allenatori",font=("Arial",20,"bold"),expand_x=True,expand_y=True,justification='left')])
            for j in trainers[floor]:
                if j["name"].lower()  not in skiptrainer or  prev_name != j["name"]:
        ...
        if floor in events and len(events[floor])>0:
            layout.append([sg.Text("Eventi",font=("Arial",20,"bold"),expand_x=True,expand_y=True,justification='left')])
