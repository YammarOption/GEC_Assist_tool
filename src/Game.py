...
class GS_Gen1 (Game):
    ...
    def checkEventForMon(self,mon:str):
        return  mon.startswith("ENTEI") or\
    ...
    def getEventForMon(self,mon:str):
        ...
        if mon.startswith("MEW"):return "MEWGLITCH"
...
class GS_Gen2 (Game):
    ...
    def renameElement(self, item:str):
        if item.startswith("COINS"):
            return "COINS"
        if item.startswith("MOM-"):
            return item.replace("MOM-","")
        ...
        if item.startswith("PUZZLE"):
            return "RUINPUZZLE"
        if item.startswith("PHONENUMBER"):
            return "PHONENUMBERS"
        if item.startswith("WEEKSIBLINGS"):
            return "WEEKSIBLINGS"
        if item.endswith("SENT"):
            return "MAILSTOPC"
        return item
