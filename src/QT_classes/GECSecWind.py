...
skiptrainer=["RIVAL","RIVAL Optional","RIVAL ???"]
...
class GECSecwindow(QMainWindow):
            ...
            for floor in floors:
                ...
                if floor in items and len(items[floor])>0:
                    ...
                    for j in items[floor]:
                        key = "ITEM-"+j
                        cbox=MyCheckbox(j.replace("PKRS_","").replace("MOM-","").replace("DECO-",""),key,parent,codecounter)
                        codecounter+=1
    ...
    def itemShow(self):
        ...
        if id.startswith("ITEM-"):
            id = id.replace("ITEM-","")
            self.parent.updateElements(id,code,state,self.currentRoute,True)
        else :
            id = id.replace("EVENT","")
            self.parent.updateElements(id,code,state,self.currentRoute,False)
