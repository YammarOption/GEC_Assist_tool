...
from PyQt5.QtWidgets import ( QSplitter,QWidget, QMenuBar, QHBoxLayout,QVBoxLayout, QGridLayout, QLabel,
    ...
    def setup(self,config:configparser.ConfigParser):
        ...
        for img in self.game.dexList:
            ...
            if img == "VUOTO":
                ...
                pic.setPixmap(image)
                self.dexlayout.addWidget(pic,int(count/self.game.MON_PER_ROW), count%self.game.MON_PER_ROW)                  
                count=count+1
                continue
        ...
        for couple in self.game.itemList:
            ...
            if item == "blank" or item == "VUOTO":
                pic=QLabel()
                image=QPixmap("Datapack/Sprites/items/blank.png")
                pic.setPixmap(image)     
                self.itemlayout.addWidget(pic,int(count/self.game.ITEMS_PER_ROW), count%self.game.ITEMS_PER_ROW)                  
                count=count+1
                continue
